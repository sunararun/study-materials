...
FROM
    interpra.department AS dept
WHERE
    LENGTH(dept.dep_name) IN (SELECT 
            COUNT(*)
        FROM
            interpra.employees AS emp1
        GROUP BY emp1.dep_id);
...
HAVING COUNT(*) IN (SELECT 
    ...
    FROM
        ...
        FROM
            interpra.employees AS emp1
        GROUP BY emp1.manager_id) empCount);
...
SELECT 
    emp.emp_name, emp.salary, emp.dep_id
FROM
    interpra.employees AS emp
WHERE
    emp.salary > ANY (SELECT 
...
SELECT 
    emp.emp_name, emp.job_name, emp.salary
FROM
    interpra.employees AS emp
WHERE
    emp.dep_id != 1001
        AND (emp.job_name , emp.salary) IN (SELECT 
            emp1.job_name, emp1.salary
        FROM
            interpra.employees AS emp1
        WHERE
            emp1.dep_id = 3001);
...
SELECT 
    ...
    emp.job_name,
    emp.salary,
    (emp.salary + emp.commission) AS 'Net Salary'
FROM
    interpra.employees AS emp
WHERE
    (emp.salary + emp.commission) IN (SELECT 
            MAX(emp1.salary + emp1.commission)
        FROM
            interpra.employees AS emp1
        WHERE
            emp1.commission != 0.00
                AND emp1.job_name = 'SALESMAN');
...
SELECT 
    ...
    emp.job_name,
    emp.salary,
    (emp.salary + emp.commission) AS 'Net_Salary'
FROM
    interpra.employees AS emp
WHERE
    2 - 1 = (SELECT 
...
*/
SELECT 
    emp.dep_id, AVG(emp.salary) AS 'avg_sal'
FROM
    interpra.employees AS emp
GROUP BY emp.dep_id
HAVING AVG(emp.salary) < ANY (SELECT 
        AVG(emp1.salary)
    FROM
        interpra.employees AS emp1);
...
SELECT 
    emp.emp_name, emp.salary, emp.commission
FROM
    interpra.employees AS emp
WHERE
